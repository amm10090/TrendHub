        ...
        <div className="flex flex-col space-y-6">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {product.sizes.map((size) => (
                            <button
                                key={size}
                                className={`min-w-12 h-10 px-3 border rounded-md text-sm transition-all ${selectedSize === size
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {product.colors.map((color) => (
                            <button
                                key={color.name}
                                className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${selectedColor === color.name
            ...
            <div className="text-xs text-text-tertiary-light dark:text-text-tertiary-dark">
                {product.availableQuantity > 0 ? (
                    <>
                        <span className="text-green-600 dark:text-green-400">{t('inStock')}</span> - {t('availableItems', { count: product.availableQuantity })}
                    </>
                ) : (
