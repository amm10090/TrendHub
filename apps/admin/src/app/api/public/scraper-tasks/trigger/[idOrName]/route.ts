...
export async function POST(
  ...
  try {
    ...
    if (!taskDefinition.isEnabled) {
      console.info(
        `[CRON_TRIGGER_POST] Task '${taskDefinition.name}' (ID: ${taskDefinition.id}) is disabled. Skipping trigger.`,
      );
    ...
    console.log(
      `[CRON_TRIGGER_POST] Attempting to enqueue task: ${taskDefinition.id} (Name: ${taskDefinition.name}) with trigger type API`,
    );
    ...
    console.error(
      "[CRON_TRIGGER_POST] Critical error while processing cron trigger:",
      error,
    );
